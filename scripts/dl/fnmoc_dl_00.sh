...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Start" "FNMOC 00z "$DATE""
../libs/tray-gribs.py "Grib2 NOAA DL — FNMOC 00z "$DATE" Start" &
...
#         echo "https://nomads.ncep.noaa.gov/pub/data/nccf/com/naefs/prod/fens."$DATE"/"$value"/pgrb2a/ENSEMBLE.MET.fcst"$pert"."$ech"."$DATE$value >> dl.txt
...
######################################
cd $DIR
python3 fnmoc_rename.py $DATE "00"
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Done" "FNMOC 00z "$DATE""
...
rm -R __pycache__
rm my.inv
echo `date +%Y%m%d` > lock/fnmoc_00.txt
