...
while [ $(cat lock/cfs_am.txt) != `date +%Y%m%d` ]
do
    sleep 5m
done
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Start" "GEFS 06z "$DATE""
../libs/tray-gribs.py "Grib2 NOAA DL — GEFS 06z "$DATE" Start" &
...
######################################
cd $DIR
python3 gefs_rename.py $DATE "06"
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Done" "GEFS 06z "$DATE""
...
rm -R __pycache__
rm my.inv
echo `date +%Y%m%d` > lock/gefs_06.txt
