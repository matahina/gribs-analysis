...
while [ $(cat lock/gefs_06.txt) != `date +%Y%m%d` ]
do
    sleep 5m
done
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Start" "CFS 12z18z "$DATE""
../libs/tray-gribs.py "Grib2 NOAA DL — CFS 12z18z "$DATE" Start" &
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Done" "CFS 12z18z "$DATE""
...
rm -R __pycache__
rm my.inv
echo `date +%Y%m%d` > lock/cfs_pm.txt
