...
while [ $(cat lock/gefs_18.txt) != `date +%Y%m%d` ]
do
    sleep 1m
done
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Start" "GEM 12z "$DATE""
../libs/tray-gribs.py "Grib2 NOAA DL — GEM 12z "$DATE" Start" &
...
######################################
cd $DIR
python3 gem_rename.py $DATE "12"
echo `date +%Y%m%d` > lock/gem_12.txt
Rscript gem_extract.R
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Done" "GEM 12z "$DATE""
