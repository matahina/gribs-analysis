...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Start" "GEFS 12z "$DATE""
../libs/tray-gribs.py "Grib2 NOAA DL — GEFS 12z "$DATE" Start" &
...
######################################
cd $DIR
python3 gefs_rename.py $DATE "12"
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Done" "GEFS 12z "$DATE""
...
rm -R __pycache__
rm my.inv
echo `date +%Y%m%d` > lock/gefs_12.txt
