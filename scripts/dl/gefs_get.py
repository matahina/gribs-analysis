...
for sc in range(1,31):
    for ech in range(0,385,6):
        f.write("https://nomads.ncep.noaa.gov/cgi-bin/filter_gefs_atmos_0p50a.pl?dir=%2Fgefs."+model_date+"%2F"+model_run+"%2Fatmos%2Fpgrb2ap5&file=gep"+"{:02d}".format(sc)+".t"+model_run+"z.pgrb2a.0p50.f"+"{:03d}".format(ech)+"&lev_2_m_above_ground=on&lev_500_mb=on&lev_850_mb=on&lev_surface=on&var_APCP=on&var_HGT=on&var_TMP=on&subregion=&toplat=50&leftlon=5&rightlon=7&bottomlat=48"+"\n")
