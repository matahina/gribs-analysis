...
while [ $(cat lock/gefs_12.txt) != `date +%Y%m%d` ]
do
    sleep 1m
done
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Start" "GEFS 18z "$DATE""
../libs/tray-gribs.py "Grib2 NOAA DL — GEFS 18z "$DATE" Start" &
...
######################################
cd $DIR
python3 gefs_rename.py $DATE "18"
echo `date +%Y%m%d` > lock/gefs_18.txt
Rscript gefs_extract.R
...
DISPLAY=:0 /usr/bin/notify-send -h "string:desktop-entry:org.kde.konsole"  "Grib2 NOAA DL Done" "GEFS 18z "$DATE""
