...
norms_pp <- data.frame(DATE = c(format(
  as.POSIXlt(norm$V5[which(format(as.POSIXlt(norm$V5, format = "%d/%m"), "%d") == "01")],
             format = "%d/%m"),
  paste0(sprintf("%s", YEAR), "-%m-%d")
),
format(
  as.POSIXlt(norm$V5[which(format(as.POSIXlt(norm$V5, format = "%d/%m"), "%d") == "01")],
             format = "%d/%m"),
  paste0(sprintf("%s", YEAR + 1), "-%m-%d")
)),
...
allruns$geop_dev <-
  allruns$geop - norm_ncep$Z500[match(vdates, norm_ncep$DATE)]
allruns$tempalt_dev <-
  allruns$tempalt - norm_ncep$T850[match(vdates, norm_ncep$DATE)]
...
for (ech in steps) {
  ...
  for (sc in pp_scens_test$runs) {
    if ((sort(test[which(test$runs == sc), "dates"])[1] != paste0(ech, "-01 00:00:00")) |
        (
          sort(test[which(test$runs == sc), "dates"], decreasing = T)[1] != format(
            as.POSIXct(paste0(ech, "-01 00:00:00")) %m+% months(1) - hours(6),
            "%Y-%m-%d %H:%M:%S"
          )
        )) {
...
plot_ppall <-
  ...
  ggtitle(sprintf(
    "PP + NORMs81-10 _ CFS 00z06z12z18z  %s > %s",
  ...
  geom_line(data = norms_pp[seq(which(norms_pp$DATE == colnames(pp_scens_sort)[1]),
                                which(norms_pp$DATE == colnames(pp_scens_sort)[length(colnames(pp_scens_sort))])), ],
            aes(x = as.POSIXct(DATE), y = PP_CUM, group = 1),
            color = "white")
```
