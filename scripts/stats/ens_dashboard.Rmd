...
norms <- data.frame(
  DATE = c(format(
    ...
    paste0(as.character(as.integer(
      substr(Sys.Date(), 1, 4)
    ) - 1), "-%m-%d %H:%M:%S")
  ),
  format(
...
if (leap_year(as.integer(substr(Sys.Date(), 1, 4)) - 1)) {
  ...
  nnorms <-
    rbind(norms[1:59, ], c(
      format(
        as.POSIXlt("29/02", format = "%d/%m"),
        paste0(as.character(as.integer(
          substr(Sys.Date(), 1, 4)
        ) - 1), "-%m-%d %H:%M:%S")
      ),
      mean(as.numeric(nnorms[c(59:60), 2])),
      mean(as.numeric(nnorms[c(59:60), 3]))
    ), norms[60:nrow(norms), ])
}
if (leap_year(as.integer(substr(Sys.Date(), 1, 4)) + 0)) {
  ...
  nnorms <-
    rbind(norms[1:424, ], c(
      ...
      mean(as.numeric(norms[c(424:425), 2])),
      mean(as.numeric(norms[c(424:425), 3]))
    ), norms[425:nrow(norms), ])
}

if (leaap == TRUE) {
  norms <- data.frame(DATE = nnorms[, 1],
...
date_var <- Sys.Date() - as.integer(commandArgs(trailingOnly = T)[3])
```
...
plotruns <-
  plotruns[which(plotruns$dates <= tail(the_dates[which(the_dates < as.POSIXct(the_dates[1]) +
                                                          days(14))], 1)), ]

pp_probs_all <-
  pp_probs_all[which(row.names(pp_probs_all) >= the_dates[1]), ]
pp_probs_all <-
  pp_probs_all[which(row.names(pp_probs_all) <= tail(the_dates[which(the_dates < as.POSIXct(the_dates[1]) +
                                                                       days(14))], 1)), ]
...
pp_probs_gefs <-
  pp_probs_gefs[which(row.names(pp_probs_gefs) <= tail(the_dates[which(the_dates < as.POSIXct(the_dates[1]) +
                                                                         days(14))], 1)), ]
...
norms_tsol <-
  data.frame(date = as.POSIXlt(c(norm$V1, norm$V3), format = "%d/%m %H:%M"),
             tsol = c(norm$V2, norm$V4))

norms_tsol_data <-
  norms_tsol[which(norms_tsol$date > the_dates[1]),][which(norms_tsol[which(norms_tsol$date > the_dates[1]), "date"] < as.POSIXct(the_dates[1]) + days(14)),]
...
pdf(
  sprintf(
    "../../data/gefs/ens_cluster_unif-%s.pdf",
